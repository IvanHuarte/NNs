...
class SymmGroup():
        ...
        for symm_name, symm_setup in setup.items():
            ...
            if 'subgroup' in symm_setup.keys():
                for subg in symm_setup['subgroup']:
                    group.append(
                        SIMMETRY[symm_name](**init_kwargs, subgroup=subg)
                    )
                    group_label.append(f"{symm_name}_{subg}")
            else:
                group.append(
                        SIMMETRY[symm_name](**init_kwargs)
                    )
    ...
    def _unitary_representation(
        self, 
        operation: callable, 
        all_states: jax.typing.ArrayLike
        ...
        for state in all_states:
            ...
            perm_idx = jnp.sum(
                jnp.abs(all_states-permuted[None, :]),
                axis=-1
            )
    ...
    def get_unitary_representations(
        self, 
        all_states: jax.typing.ArrayLike
    )-> List:
        """
