...
def disjoint_avg_pool(x: jt.ArrayLike, strides: Tuple) -> jt.ArrayLike:
...
class CvTWorker(nn.Module):
    ...
    strides_list: Tuple[
        Tuple[int, int], ...
    ]  # Stride value for each stage. Applied in CTE
    kernel: Tuple = (
        3,
        3,
    )  # Kernel size for the convolutional operations (must be 3x3 or 3x1 for 1D)
