...
def display_diagnosis_sanity_monitor(diagnosis: dict, verbose: int = 3) -> None:
    ...
    if has_gradients:
        ...
        if 'global_norm' in gradients_data:
            ...
            if should_display_status(gn['status']):
                if verbose == 4:
                    left_content.append(f"  {gn['status']}", style=THEME['text_primary'])
                    messages_dict[message_counter] = gn['message']
                    left_content.append(f"   ({message_counter})\n", style=THEME['text_dim'])
                    message_counter += 1
                else:
                    left_content.append(f"  {gn['status']}\n", style=THEME['text_primary'])
        ...
        if 'norm_per_sample' in gradients_data:
            ...
            if 'mean' in nps:
                ...
                if should_display_status(mean_data['status']):
                    if verbose == 4:
                        left_content.append(f"  {mean_data['status']}", style=THEME['text_primary'])
                        messages_dict[message_counter] = mean_data['message']
                        left_content.append(f"   ({message_counter})\n", style=THEME['text_dim'])
                        message_counter += 1
                    else:
                        left_content.append(f"  {mean_data['status']}\n", style=THEME['text_primary'])
            ...
            if 'std' in nps:
                ...
                if should_display_status(std_data['status']):
                    if verbose == 4:
                        left_content.append(f"  {std_data['status']}", style=THEME['text_primary'])
                        messages_dict[message_counter] = std_data['message']
                        left_content.append(f"   ({message_counter})\n", style=THEME['text_dim'])
                        message_counter += 1
                    else:
                        left_content.append(f"  {std_data['status']}\n", style=THEME['text_primary'])
        ...
        if 'ReIm_norms' in gradients_data:
            ...
            for key in ['global', 'ModulusNet', 'PhaseNet']:
                ...
                if key in reim:
                    reim_data = reim[key]
                    left_content.append(f"{key.capitalize()}:   ", style=THEME['subtitle_color'])
                    left_content.append(reim_data['label'], style=THEME['text_primary'])
                    if should_display_status(reim_data['status']):
                        if verbose == 4:
                            left_content.append(f"  {reim_data['status']}", style=THEME['text_primary'])
                            messages_dict[message_counter] = reim_data['message']
                            left_content.append(f"   ({message_counter})\n", style=THEME['text_dim'])
                            message_counter += 1
                        else:
                            left_content.append(f"  {reim_data['status']}\n", style=THEME['text_primary'])
        ...
        if 'percentage_norm_intervals' in gradients_data:
            ...
            if should_display_status(pni['status']):
                ...
                if verbose == 4:
                    ...
                    right_content.append(f"{pni['status']}", style=THEME['text_primary'])
                    right_content.append(f"   ({message_counter})\n", style=THEME['text_dim'])
                    message_counter += 1
                else:
                    right_content.append(f"{pni['status']}\n", style=THEME['text_primary'])
            ...
            if isinstance(label_text, dict):
                for interval_name, percentage in label_text.items():
                    ...
                    right_content.append(f"{interval_name}: ", style=THEME['text_primary'])
        ...
        gradient_panel_content = Columns([
            Panel(left_content, title="[bold][/bold]", border_style=THEME['text_primary'], box=box.SIMPLE, padding=(1, 2)),
            Panel(right_content, title="[bold][/bold]", border_style=THEME['text_primary'], box=box.SIMPLE, padding=(1, 2))
        ], equal=True, expand=True)
        
        # Get status for gradients title
        gradient_status = gradients_data.get('overall', {}).get('status', '🟢') if 'overall' in gradients_data else '🟢'
        ...
        for left_metric, right_metric in metric_pairs:
            ...
            for submetric_name, submetric_data in left_metric_data.items():
                ...
                if isinstance(submetric_data, dict) and 'status' in submetric_data:
                    left_text.append(submetric_data['label'], style=THEME['text_primary'])
                    if should_display_status(submetric_data['status']):
                        if verbose == 4:
                            messages_dict[message_counter] = submetric_data['message']
                            left_text.append(f"  {submetric_data['status']}", style=THEME['text_primary'])
                            left_text.append(f"  ({message_counter})\n", style=THEME['text_dim'])
                            message_counter += 1
                        else:
                            left_text.append(f"  {submetric_data['status']}\n", style=THEME['text_primary'])

                        left_text.append("\n")
                
                elif isinstance(submetric_data, dict):

                    for subsubmetric_name, subsubmetric_data in left_metric_data.items():

                        if isinstance(subsubmetric_data, dict) and 'status' in subsubmetric_data:
                            left_text.append(subsubmetric_data['label'], style=THEME['text_primary'])
                            if should_display_status(subsubmetric_data['status']):
                                if verbose == 4:
                                    messages_dict[message_counter] = subsubmetric_data['message']
                                    left_text.append(f"  {subsubmetric_data['status']}", style=THEME['text_primary'])
                                    left_text.append(f"  ({message_counter})\n", style=THEME['text_dim'])
                                    message_counter += 1
                                else:
                                    left_text.append(f"  {subsubmetric_data['status']}\n", style=THEME['text_primary'])

                                left_text.append("\n")

                else:
                    raise Warning(f"Incorrect data format for metric '{submetric_name}' in '{left_metric}'")
            ...
            # ========== CREATE RIGHT PANEL ==========
            if right_metric:
                right_text = Text(justify="center",)
                right_metric_data = diagnosis[right_metric]
                for submetric_name, submetric_data in right_metric_data.items():

                    if isinstance(submetric_data, dict) and 'status' in submetric_data:
                        right_text.append(submetric_data['label'], style=THEME['text_primary'])
                        if should_display_status(submetric_data['status']):
                            if verbose == 4:
                                messages_dict[message_counter] = submetric_data['message']
                                right_text.append(f"  {submetric_data['status']}", style=THEME['text_primary'])
                                right_text.append(f"  ({message_counter})\n", style=THEME['text_dim'])
                                message_counter += 1
                            else: 
                                right_text.append(f"  {submetric_data['status']}\n", style=THEME['text_primary'])
                        ...
                        for subsubmetric_name, subsubmetric_data in left_metric_data.items():
                            ...
                            if isinstance(subsubmetric_data, dict) and 'status' in subsubmetric_data:
                                right_text.append(subsubmetric_data['label'], style=THEME['text_primary'])
                                if should_display_status(subsubmetric_data['status']):
                                    if verbose == 4:
                                        messages_dict[message_counter] = subsubmetric_data['message']
                                        right_text.append(f"  {subsubmetric_data['status']}", style=THEME['text_primary'])
                                        right_text.append(f"  ({message_counter})\n", style=THEME['text_dim'])
                                        message_counter += 1
                                    else:
                                        right_text.append(f"  {subsubmetric_data['status']}\n", style=THEME['text_primary'])
    ...
    if verbose == 4 and messages_dict:
        ...
        for msg_num in sorted(messages_dict.keys()):
            ...
            messages_text.append(f" → {messages_dict[msg_num]}\n", style=THEME['text_primary'])
    ...
    console.print("\n" + "─" * 80)
    overall_panel = Panel(
        Text(f"{overall_status} {overall_message}", justify="center", style=THEME['subtitle_color']),
        title="[bold]OVERALL[/bold]",
        border_style=THEME['error'] if overall_status == "🔴" else THEME['warnings'] if "⚠️" in overall_status else THEME['ok'],
        padding=(1, 2)
    )
...
def display_diagnosis_simple(diagnosis: dict, verbose: int = 3):
    ...
    if 'gradients' in diagnosis:
        ...
        if should_show(g_status):
            ...
            print(f"  {g_status} Global: {g_label}" + (f" ({msg_num})" if msg_num else ""))
        ...
        print("  📈 Per-sample:")
        nmean_label, nmean_status, nmean_msg = safe_extract(grad['norm_per_sample']['mean'])
        if should_show(nmean_status):
            msg_num = msg_counter + 1 if verbose == 4 else None
            print(f"    {nmean_status} Mean: {nmean_label}" + (f" ({msg_num})" if msg_num else ""))
        ...
        if should_show(nstd_status):
            msg_num = msg_counter + 1 if verbose == 4 else None
            print(f"    {nstd_status} Std:  {nstd_label}" + (f" ({msg_num})" if msg_num else ""))
        ...
        for net_name in ['global', 'ModulusNet', 'PhaseNet']:
            ...
            if should_show(r_status):
                msg_num = msg_counter + 1 if verbose == 4 else None
                print(f"    {r_status} {net_name}: {r_label}" + (f" ({msg_num})" if msg_num else ""))
    ...
    if 'sampling' in diagnosis:
        ...
        for subkey, subdata in diagnosis['sampling'].items():
            ...
            if should_show(s_status):
                msg_num = msg_counter + 1 if verbose == 4 else None
                print(f"  {s_status} {subkey}: {s_label}" + (f" ({msg_num})" if msg_num else ""))
    ...
    if 'phase' in diagnosis:
        ...
        for subkey, subdata in diagnosis['phase'].items():
            ...
            if should_show(p_status):
                msg_num = msg_counter + 1 if verbose == 4 else None
                print(f"  {p_status} {subkey}: {p_label}" + (f" ({msg_num})" if msg_num else ""))
...
if __name__ == "__main__":
    ...
    diagnosis_example = {
        'gradients': {
            ...
            'percentage_norm_intervals': {
                'label': ['   > 1e+03:        0.000%', '1e+02 <--> 1e+03:   0.000%', '1e+01 <--> 1e+02:   0.000%', 
                        '1e+00 <--> 1e+01:   8.501%', '1e-01 <--> 1e+00:   42.733%','1e-02 <--> 1e-01:   40.754%', 
                        '1e-03 <--> 1e-02:   6.953%', '1e-04 <--> 1e-03:   0.955%', '    < 1e-04:        0.104%'] ,
                'status': '🟢',
