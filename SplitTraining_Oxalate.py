...
strength = config_cm["CM"][cm_model_name]["strength"]  # Lattice and coupling model
theta_list = config_cm["CM"][cm_model_name]["theta_list"]
phi_list = config_cm["CM"][cm_model_name]["phi_list"]
...
for i, size in enumerate(sizes):
    ...
    for j, (theta, phi) in enumerate(zip(theta_list, phi_list)):
        ...
        config_cm["CM"][cm_model_name]["theta"] = theta
        config_cm["CM"][cm_model_name]["phi"] = phi
        config_cm["size"] = size
        display_simulation_settings({**config_cm, **config_nn})

        ## Update Hamiltonian
        oxa = OxalateJKGamma(size, [strength, theta, phi], **kwargs_lattice)
