...
for trivial in [True, False]:                                   # LOOP OVER TRIVIAL AND NON TRIVIAL

    if 'trivial_Z2' in nn_model_setup.keys():
        nn_model_setup['trivial_Z2']= trivial
    ...
    for i, size in enumerate(sizes):
        ...
        for j, J in enumerate(J_list):
            ...
            for sweeps in schedule['sweeps']:
                ...
                fidelity = np.abs(np.vdot(vstate.to_array(), x_ED.squeeze())) if exact_diag else None
                print(f"Fidelity: {fidelity:.4e}")
