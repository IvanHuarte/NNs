...
for sweep in sweep_cases:
    ...
    for size in sizes:
        ...
        alphas = set(
            [re.search(r"alpha_([\d\.]+)", file).group(0) for file in json_objects]
        )

        for alpha in alphas:
            alpha_files = [file for file in json_objects if alpha in file]
            if not alpha in tree_files[sweep].keys():
                tree_files[sweep][alpha] = {}
...
for sweep, sweep_dict in tree_files.items():
    ...
    for alpha, alpha_dict in sweep_dict.items():
        ...
        for size, files in alpha_dict.items():
            ...
            results = [
                [] for _ in range(16)
            ]  # For [J, E_best, E_ED, error, vscore, S_renyi, M, Ms, fidelity, timexe]
            ...
            ax1[0].set_title(
                r"$%s \qquad \alpha=%.2f \quad %s \;sweeps$"
                % (model_name, alpha_num, sweep),
            ...
            ax2[0].set_title(
                r"$%s \qquad  \alpha=%.2f$ " % (model_name, alpha_num),
                fontsize=fontsize_title,
            )
        ...
        fig1.savefig(
            parent_folder
            + f"RunInJ_sweep_{sweep}_alpha_{alpha_num}_Energy_Error_Vscore"
            + ".jpeg",
            dpi=600,
            bbox_inches="tight",
        )
        fig2.savefig(
            parent_folder
            + f"RunInJ_sweep_{sweep}_alpha_{alpha_num}_Renyi_Mz_Ms"
            + ".jpeg",
            dpi=600,
            bbox_inches="tight",
        )
        fig3.savefig(
            parent_folder
            + f"RunInJ_sweep_{sweep}_alpha_{alpha_num}_Fidelity_Timexe"
            + ".jpeg",
            dpi=600,
            bbox_inches="tight",
        )
