...
for size in sizes:
    ...
    for params in model_factory.get_params():
        ...
        write_folder = write + f"{cm_model_name}/" + f"Size_{size[0]}x{size[1]}/" + sim_label + "/"
        ...
        for i, (fig, ax) in enumerate(batch_modphase_plotter(x_ED[:k], labels=labels[:k], return_figure=True)):
            fig.savefig(write_folder + "Modphase/" + f"{labels[i].replace(' ', '_')}{sim_label}.png", dpi=600)
        ...
        fig_cum.savefig(write_folder + f"Cumulative_contributors{sim_label}.png", dpi=600)


        # Weighted phase histogram
        fig_phase, ax_phase = plot_weighted_phase_hist(x_ED[0])
        fig_phase.suptitle(title, fontsize=30)
        fig_phase.savefig(write_folder + f"Weighted_phase_histogram{sim_label}.png", dpi=600)
